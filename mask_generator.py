...
class FCDMaskGenerator():
    ...
    def __init__(self, h, w, nb_of_dims, latent_dim, use_coronal, use_sagital, p, experiment_name, parallel, model_weights='best_model.pth',  thr=.5):
        ...
        self.transform = torchio.transforms.Compose([histogram_transform, znorm_transform])
            ...
            for j in range(0, self.gmpm.shape[1], self.h):
                ...
                if start_idx < mid_idx:
                    patch_1_axial = np.stack((
                        ...
                        target_np[-start_idx - 1: -start_idx - self.w - 1: -1, j: j + self.h, i], #mirrored axial slice 
                    ))       

                    patch_1_coronal = np.stack((
                        target_np[start_idx: start_idx + self.w, j + self.h//2, i - self.h//2: i + self.h//2], #coronal slice   
                        target_np[-start_idx - 1: -start_idx - self.w - 1: -1, j + self.h//2, i - self.h//2: i + self.h//2] #mirrored coronal slice  
                    ))

                    patch_1_sagital = np.stack((
                        target_np[start_idx + self.w//2, j: j + self.h, i - self.w//2: i + self.w//2], #sagital  slice 
                        target_np[-start_idx - self.w//2, j: j + self.h, i - self.w//2: i + self.w//2], #mirrored sagital slice                  
                    ))
                    ...
                    patch_2_coronal = np.stack((
                        ...
                        target_np[start_idx + self.w - 1: start_idx - 1: -1, j + self.h//2, i - self.h//2: i + self.h//2], 
                    ))
                ...
                if not (i <= 44 and j >= 118): 
                    ...
                    patch_3_coronal = np.stack((
                        ...
                        target_np[mid_idx + 2*self.w - 1: mid_idx + self.w - 1: -1, j + self.h//2, i - self.h//2: i + self.h//2] 
                    ))
                    ...
                    patch_4_coronal = np.stack((
                        target_np[mid_idx + self.w: mid_idx + 2*self.w,  j + self.h//2, i - self.h//2: i + self.h//2], 
                        target_np[mid_idx + self.w - 1: mid_idx - 1 : -1, j + self.h//2, i - self.h//2: i + self.h//2]
                    ))
        ...
        for i in range(self.gmpm.shape[2]):
            ...
            for j in range(0, self.gmpm.shape[1], self.h):
                ...
                if not (i <= 44 and j >= 118): 
                    ...
                    mid_mask_np[mid_idx + self.w : mid_idx + 2*self.w, j:j + self.h, i] = patch_map_tensor[2, j//self.h, i]
    ...
    def detection_pipeline(self, input_img_name, input_mask_name=None, save_mask_name='pred_mask.nii.gz', probs=False):
        ...
        if not probs:
            ...
            if save_mask_name is not None:
                self.save_nii_mask(side_mask_np, img, os.path.join(f'./data/predicted_masks/{self.experiment_name}', 'side_'+ os.path.basename(save_mask_name)))
                self.save_nii_mask(mid_mask_np, img, os.path.join(f'./data/predicted_masks/{self.experiment_name}', 'mid_' + os.path.basename(save_mask_name)))
