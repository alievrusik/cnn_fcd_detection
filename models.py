...
class ConvolutionBlock(nn.Module):
    def __init__(self, in_c, out_c, pad=1, p=0., max_pool=False):
        super().__init__()
        self.conv = nn.Conv2d(in_c, out_c, kernel_size=3, padding=pad, bias=False)
...
class DeConvolutionBlock(nn.Module):
    def __init__(self, in_c, out_c, output_pad=0, pad=0, stride=1, p=0., use_relu=True):
        super().__init__()
        self.conv = nn.ConvTranspose2d(in_c, out_c, kernel_size=3,
                                       output_padding=output_pad, padding=pad, stride=stride, bias=False)
        self.bn = nn.BatchNorm2d(out_c)
        if use_relu:
            self.relu = nn.ReLU()
        else:
...
def get_conv_branch(p, input_dim):
    return nn.Sequential(
            ConvolutionBlock(input_dim, 16),
...
def get_deconv_branch(p, input_dim):
    return nn.Sequential(
            ...
            DeConvolutionBlock(16, input_dim, use_relu=False, pad=1)
        )


class Encoder(nn.Module):
    def __init__(self, h, w, mods, nb_of_dims, latent_dim, p=.4):
        super().__init__()
        self.nb_of_dims = nb_of_dims
        self.conv_blocks = nn.ModuleList()
        self.mods = mods
        for _ in range(nb_of_dims):
            self.conv_blocks.append(get_conv_branch(p=p, input_dim=2*mods))

        self.dropout = nn.Dropout(p=p)
        self.fc1 = nn.Linear(nb_of_dims * h//8 * w//8 * 256, latent_dim, bias=False)
    ...
    def forward(self, x):
        ...
        for i in range(self.nb_of_dims):
            x_after_conv.append(self.conv_blocks[i](x[:, 2*i*self.mods: 2*(i+1)*self.mods]))
...
class Decoder(nn.Module):
    def __init__(self, h, w, mods, nb_of_dims, latent_dim, p=.4):
        super().__init__()  
        self.fc1 = nn.Linear(latent_dim, nb_of_dims * h//8 * w//8 * 256, bias=False)
        ...
        for _ in range(nb_of_dims):
            self.deconv_blocks.append(get_deconv_branch(p=p, input_dim=2*mods))
...
class PatchAutoEncoder(nn.Module):
    ...
    def __init__(self, h, w, mods, nb_of_dims, latent_dim, p):
        super().__init__()
        self.encoder = Encoder(h, w, mods, nb_of_dims, latent_dim, p=p)
        self.decoder = Decoder(h, w, mods, nb_of_dims, latent_dim, p=p)
